...
class ModelBuilder(nn.Module):
    def __init__(self, net_cfg):
        ...
        if net_cfg["is_encoder"]:
            self.encoder = self._build_encoder(net_cfg["encoder"])
        else:
            self.encoder = self._build_encoder(net_cfg["encoder1"])
            
        if net_cfg["is_decoder50"]:
            self.decoder = self._build_decoder(net_cfg["decoder"])
        else:
            self.decoder = self._build_decoder(net_cfg["decoder1"])
    ...
    def forward(self, x):
        ...
        if self._use_auxloss:
            ...
            feat = self.encoder(x) # feat = x1 x2 x3 x4
            # 这里添加注意力机制
            # print("x.shape=",feat.size) 
